...
<body>
    ...
    <div class="main-container">
        <div class="container-div">
            <div class="tabs">
                ...
                <div class="tab" data-status="Pending">Menunggu</div>
                <div class="tab" data-status="Processing">Diproses</div>
                <div class="tab" data-status="On the way">Dalam Perjalanan</div>
                <div class="tab" data-status="Completed">Selesai</div>
                <div class="tab" data-status="Cancelled">Dibatalkan</div>
            </div>
            <div id="orders">
    ...
    <div id="cancelModal" class="modal">
        <div class="modal-content">
            ...
            <button id="cancelOrderBtn">Batalkan Pesanan</button>
        </div>
    </div>
    ...
    <div id="reviewModal" class="modal">
        <div class="modal-content">
            ...
            <form id="reviewForm" action="submit_reviews.php" method="POST">
                ...
                <br>
                <button type="submit" id="submitReviewBtn" class="review-btn">Kirim Ulasan</button>
            </form>
        </div>
    </div>
...
?>
    ...
    <script>
        ...
        function getPaymentModeTranslation(pmode) {
            switch (pmode) {
                case 'Takeaway': return 'Ambil di tempat';
                case 'Cash': return 'Tunai (Cash)';
                case 'Card': return 'Transfer (QRIS)';
                    ...
                    ordersContainer.innerHTML = data.map(order => `
                        <div class="order container" style="padding: 20px 30px;">
                            <div class="order-header" style="font-size: 1.3rem;">
                                <div>ID Pesanan: #${order.order_id}</div>
                                <div class="status ${getStatusClass(order.order_status)}">Status: <span class="status-text">${getStatusTranslation(order.order_status)}</span></div>
                            </div>
                            <div class="order-details">
                                <div class="customer-details">
                                    <div><p><strong>Nama: </strong></p></div>
                                    <div><p>${order.firstName} ${order.lastName}</p></div>
                                </div>
                                <div class="customer-details">
                                    <div><p><strong>Alamat: </strong></p></div>
                                    <div><p>${order.address}</p></div>
                                </div>
                                <div class="customer-details">
                                    <div><p><strong>Kontak: </strong></p></div>
                                    <div><p>${order.phone}</p></div>
                                </div>
                                <div class="customer-details">
                                    <div><p><strong>Metode Pembayaran: </strong></p></div>
                                    <div><p>${getPaymentModeTranslation(order.pmode)}</p></div>
                                </div>
                                <div class="customer-details">
                            ...
                            <div class="order-items" style="font-size: 1.1rem;">
                                ...
                                `).join('')}
                                 <div class="order-total">Total Keseluruhan: ${order.grand_total}</div>
                        ${order.order_status === 'Cancelled' ? `
                        <div class="review mt-3">
                        <div><p><strong>Alasan Pembatalan: </strong></div>
                   ...
                   ${order.order_status !== 'Completed' && order.order_status !== 'Cancelled' ? `<button class="cancel-btn" onclick="openCancelModal(${order.order_id})">Batalkan Pesanan</button>` : ''}
                    ...
                    ${(order.order_status === 'Completed' || order.order_status === 'Cancelled') && order.review_text ? `
                        <div class="review-section">
                         <div class=" review">
                            <div><p><strong>Ulasan Anda: </strong></p></div>
                            <div><p><span>${order.review_text}</span></p></div>
                         </div>
                            ${order.response ? `
                            <div class=" review">
                              <div><p><strong>Tanggapan: </strong></p></div>
